...
int convert_file(char *filename_output, char *filename_input, int compression_flag)
{
    ...
    */
    ...
    const char *MeasureLength_str = "MeasureLength[m]";
    const char *SpatialResolution_str = "SpatialResolution[m]";
    bool find_MeasureLength = false, find_SpatialResolution = false;
    long int offset_before_properties = ftell(fp);
    ...
    for (int ii = 0; ii < number_of_properties; ii++)
    {
        ...
        case tdsTypeU32:
            ...
            if (!strcmp(MeasureLength_str, properties_name))
            {
                find_MeasureLength = true;
        ...
        case tdsTypeDoubleFloat:
            ...
            if (!strcmp(SpatialResolution_str, properties_name))
            {
                find_SpatialResolution = true;
    ...
    if (transpose_flag)
    {
        ...
        dataset_id = H5Dcreate(file_id, "/Acoustic", H5T_STD_I16LE, dataspace_id, H5P_DEFAULT, dataset_plist_id, H5P_DEFAULT);
     ...
     */
    fseek(fp, offset_before_properties, SEEK_SET);
    std::string ss;
    for (int ii = 0; ii < number_of_properties; ii++)
    {
        //Name Length and name
   ...
   */
    ...
    hid_t metadata_dset = H5Dcreate(file_id, "/AllHeaderFromTDMS", metadata_dtype, metadata_space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
