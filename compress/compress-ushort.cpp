...
inline Stencil<std::vector<unsigned short>> udf_compress(const Stencil<short> &iStencil)
{
	...
	Stencil<std::vector<unsigned short>> oStencil;
	//
	// Deal with tag
	//
	...
	PrintVector("ts_short = ", ts_short);

	std::vector<unsigned short> ts_unsigned_short;
	ts_unsigned_short.resize(ts_short.size());
	for (size_t i = 0; i < ts_short.size(); i++)
	{
		ts_unsigned_short[i] = std::abs(ts_short[i]);
	}
	PrintVector("ts_unsigned_short = ", ts_unsigned_short);
...
int main(int argc, char *argv[])
{
	...
	if (is_compression && is_preprocessing)
	{
...
int read_config_file(std::string file_name, int mpi_rank)
{
	...
	temp_str = reader.Get("parameter", "is_compression", "false");
	is_compression = (temp_str == "false") ? false : true;
