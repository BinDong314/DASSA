template <typename T> void detrend(T *y, int m) {
  T xmean = 0, ymean = 0;
  T Sxy = 0;
  T Sxx = 0;
  T grad, yint;

  // Use std::vector instead of unique_ptr to manage x
  std::vector<T> x(m);

  /********************************
  Set the X axis Linear Values
  *********************************/
  for (int i = 0; i < m; i++)
    x[i] = i;
...
template <typename T>
inline void detrend_range(const std::vector<T> &y, const size_t start,
                          const size_t count, const std::vector<T> &ctap,
                          const double xmean, const double Sxx,
  ...
  *********************************/
  ...
  // std::cout << "yint = " << yint << " , grad = " << grad << ", Sxy = " << Sxy
  // << ", Sxx = " << Sxx << ", xmean = " << xmean << ", ymean = " << ymean << "
  // \n"; PrintVector("After detrend_range out_vector =", out_vector);
}

template <typename T>
inline double subset_sqsum(const std::vector<T> &y, const size_t start,
                           const size_t count, std::vector<T> &out_vector) {
...
template <typename T>
inline void sum_sqrt(const std::vector<T> &x, std::vector<T> &sumsq,
...
template <typename T>
inline void
detrend_range_one_pass_std(const std::vector<T> &y, const size_t start,
                           const size_t count, const std::vector<T> &ctap,
                           const double xmean, const double xsum,
                           const double Sxx, std::vector<T> &out_vector) {
...
template <typename T>
inline void detrend_range_dqueue(const std::deque<T> &y, const size_t count,
                                 const std::vector<T> &ctap, const double xmean,
                                 const double Sxx, std::vector<T> &out_vector) {
