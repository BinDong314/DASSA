...
std::vector<double> bfsos1 = {
    1, 2,  1, 1, 0.502206458992975, 0.225842674724860,
...
std::vector<std::vector<std::vector<double>>>
    template_data; //[template index][channel][points]
...
std::vector<double> cheby1_b = {
    3.58632426674156e-09, 2.86905941339325e-08, 1.00417079468764e-07,
    2.00834158937527e-07, 2.51042698671909e-07, 2.00834158937527e-07,
    1.00417079468764e-07, 2.86905941339325e-08, 3.58632426674156e-09};
std::vector<double> cheby1_a = {1,
                                -7.39772047094363,
                                24.0727277609670,
                                -44.9989146659833,
                                52.8434667669224,
...
void all_gather_vector(const std::vector<double> &v_to_send,
                       std::vector<double> &v_to_receive);

// bool is_correlation_method = false;
int correlation_method =
    0; // correlation_method = 0 (dot-product), 1 (xcorr-max), 2 (fft-max)
...
void init_xcorr() {
  ...
  nseg =
      round(86400 /
            tsegment); // IF EACH SEGMENT IS 1200 S, WE WILL PROCESS 72 SEGMENTS
                       // IN A DAY 86400 = (24 hours *60 minutes *60 seconds)
  nof1 = round(tsegment / (dt0 * npts0)) +
         1; // 21, NUMBER OF FILES IT WILL PROCESS AT ONE TIME, ADDING 1

  // ctap0=tukeywin((nof1*npts0),5/(nof1*npts0*dt0));
  tukeywin(ctap0, nof1 * npts0, 5 / (nof1 * npts0 * dt0));

  if (!ft_rank) {
    PrintScalar("taperwidth / (nof1 * npts0 * dt0) = ",
                5 / (nof1 * npts0 * dt0));
  ...
  tukeywin(ctap_template1, (npts0 * nlen_template),
           5 / (npts0 * nlen_template * dt0));
  if (!ft_rank)
  ...
  std::vector<std::string> control_para_ve, aug_merge_index,
      aug_input_search_rgx, file_size_str, null_str;
  ...
  std::vector<std::string> tstart_input_template_files_new,
      winlen_input_template_files_new, h5_input_template_files_new,
      smoothhw_input_template_files_new;
  if (is_input_template_file_list) {
    ...
    for (int i = 0; i < n_input_template_files; i++) {
      tstart_input_template_files_new[i] =
          "tstart_" + input_template_files[i] + ".txt"; // tstart_ci37308124.txt
      winlen_input_template_files_new[i] =
          "winlen_" + input_template_files[i] + ".txt"; // winlen_ci37308124.txt
      h5_input_template_files_new[i] =
          input_template_files[i] + ".h5"; // ci37308124.h5
      if (is_smoothhw)
        smoothhw_input_template_files_new[i] =
            "smoothhw_" + input_template_files[i] +
            ".txt"; // smoothhw_ci37329164.txt
    }
  }
  ...
  if (!is_input_template_file_list) {
    if (!is_template_input_search_rgx) {
      ...
      aug_input_search_rgx.push_back("(.*)tstart(.*)" +
                                     template_input_search_rgx + "(.*)txt$");
    }
    T_tsstart->EndpointControl(DIR_INPUT_SEARCH_RGX, aug_input_search_rgx);
  } else {
    T_tsstart->ControlEndpoint(DIR_SET_INPUT_FILE_LIST,
                               tstart_input_template_files_new);
  }
  ...
  if (!is_input_template_file_list) {
    if (!is_template_input_search_rgx) {
      aug_input_search_rgx_winlen.push_back(
          "(.*)winlen(.*)txt$"); // tstart_ci39534271.txt
    } else {
      aug_input_search_rgx_winlen.push_back(
          "(.*)winlen(.*)" + template_input_search_rgx +
          "(.*)txt$"); // tstart_ci39534271.txt
    }
    T_winlen->EndpointControl(DIR_INPUT_SEARCH_RGX,
                              aug_input_search_rgx_winlen);
  } else {
    T_winlen->ControlEndpoint(DIR_SET_INPUT_FILE_LIST,
                              winlen_input_template_files_new);
  }
  ...
  std::vector<std::string> aug_input_search_rgx_smoothhw,
      file_size_str_smoothhw;
  ...
  if (is_smoothhw) {
    ...
    if (!is_input_template_file_list) {
      if (!is_template_input_search_rgx) {
        aug_input_search_rgx_smoothhw.push_back(
            "(.*)smoothhw(.*)txt$"); // tstart_ci39534271.txt
      } else {
        aug_input_search_rgx_smoothhw.push_back(
            "(.*)smoothhw(.*)" + template_input_search_rgx +
            "(.*)txt$"); // tstart_ci39534271.txt
      }
      T_smoothhw->EndpointControl(DIR_INPUT_SEARCH_RGX,
                                  aug_input_search_rgx_smoothhw);
    } else {
      T_smoothhw->ControlEndpoint(DIR_SET_INPUT_FILE_LIST,
                                  smoothhw_input_template_files_new);
    }
  ...
  if (!is_input_template_file_list) {
    if (!is_template_input_search_rgx) {
      ...
      aug_input_search_rgx_h5.push_back("(.*)" + template_input_search_rgx +
                                        "(.*)h5$"); // tstart_ci39534271.txt
    }
  ...
  if (ft_size > 1) {
    ...
    if (!ft_rank)
      std::cout << "rank [" << ft_rank
                << "]: my_chunk_start = " << my_chunk_start
                << ", my_chunk_end = " << my_chunk_end << "\n";
  ...
  if (!ft_rank)
    std::cout << "Rank " << ft_rank << "ntemplates = " << ntemplates
              << ", ntemplates_to_go = " << ntemplates_to_go << std::endl;
  ...
  for (int rc2 = 0; rc2 < ntemplates_to_go; rc2++) {
    ...
    tukeywin(
        ctap_template2, template_winlen[rc2],
        0.04); // Put outside if template_winlen[rc2] is constent across
               // tempaltes PrintVector("ctap_template2 = ", ctap_template2);

    // std::cout << " template_tstart[" << rc2 << "].size() = " <<
    // template_tstart[rc2].size() << " \n"; std::cout << " template_weights["
    // << rc2 << "].size() = " << template_weights[rc2].size() << " \n";
    // std::cout << "T_chs = " << T_chs << " \n";

    T_ts2d = DasLib::Vector1D2DByColStride(
        T_chs, T_h5_data, 2,
        3); // filter the data starting at ch (2-1) and every 3 chs
...
#pragma omp parallel for
    ...
    for (int i = 0; i < T_ts2d.size(); i++) {
      ...
      //             {
      //                 printf("Init Inside the OpenMP parallel region thread
      //                 0, we have %d threads, at template %d of MPI rank %d
      ...
      atemp2 = ddff(T_ts2d[i], ctap_template1, 10, BUTTER_A, BUTTER_B, cheby1_b,
                    cheby1_a);
      ...
      slice(atemp2, template_tstart[rc2][i] - 1,
            template_tstart[rc2][i] - 1 + template_winlen[rc2] - 1);
      // std::cout << " atemp2.size() = " << atemp2.size() << ",
      // template_tstart[rc2][i] = " << template_tstart[rc2][i] << ",
      // template_winlen[rc2] = " << template_winlen[rc2] << ",
      // ctap_template2.size= " << ctap_template2.size() << "\n";
    ...
    double template_tstart_min = *(std::min_element(
        template_tstart[rc2].begin(), template_tstart[rc2].end()));
  ...
  if (ft_size > 1) {
    ...
    // std::vector<std::vector<std::vector<double>>> template_data; //[template
    // index][channel][points] std::vector<double> template_winlen;
    // std::vector<std::vector<double>> template_tstart;  //[template
    // index][channel] std::vector<std::vector<double>> template_weights;
    // //[template index][channel]

    // std::cout << "Rank " << ft_rank << " template_data.size() = " <<
    // template_data.size() << "\n"; if (template_data.size() > 0)
    // {
    //     std::cout << "Rank " << ft_rank << ": template_data[0].size() = " <<
    //     template_data[0].size() << "\n";
    // }
    ...
    MPI_Allreduce(&channels, &max_channels, 1, MPI_INT, MPI_MAX,
                  MPI_COMM_WORLD);
    MPI_Allreduce(&timepoints, &max_timepoints, 1, MPI_INT, MPI_MAX,
                  MPI_COMM_WORLD);
    ...
    template_data = Convert1DVTo3DV(template_data_1D_merged, ntemplates,
                                    max_channels, max_timepoints);
    ...
    template_tstart = ConvertVector1DTo2D(template_tstart_1D_merged, ntemplates,
                                          max_channels, true);
    ...
    template_weights = ConvertVector1DTo2D(template_weights_1D_merged,
                                           ntemplates, max_channels, true);
  ...
  if (is_save_pre_processed_files && !ft_rank) {
    std::vector<int> data_chunk_size_p(2, 0);
    std::vector<int> data_overlap_size_p(2, 0);
    int n_templates = template_data.size();
    for (int template_index = 0; template_index < n_templates;
         template_index++) {
      AU::Array<double> *T = new AU::Array<double>(
          "EP_HDF5:template-intermediate-" + std::to_string(template_index) +
          ".h5:/template");
      ...
      end_address[0] = T_size[0] - 1;
      std::vector<double> template_data_1D =
          Convert2DVTo1DV(template_data[template_index]);
      std::vector<double> template_data_1D_transposed;
      template_data_1D_transposed.resize(template_data_1D.size());
      transpose(template_data_1D.data(), template_data_1D_transposed.data(),
                T_size[1], T_size[0]);
      T->WriteArray(start_address, end_address, template_data_1D_transposed);

      AU::Array<double> *W =
          new AU::Array<double>("EP_HDF5:template-intermediate-" +
                                std::to_string(template_index) + ".h5:/weight");
      std::vector<unsigned long long> W_size(1, 0);
      // W_size[0] = template_weights.size();
      W_size[0] = template_weights[template_index].size();
      data_chunk_size_p.resize(1, 0);
      data_overlap_size_p.resize(1, 0);
      W->CreateEndpoint(W_size, data_chunk_size_p, data_overlap_size_p);
      start_address.resize(1, 0);
      end_address.resize(1, 0);
      end_address[0] = W_size[0] - 1;
      // end_address[1] = W_size[1] - 1;
      // std::vector<double> template_weights_1D =
      // Convert2DVTo1DV(template_weights);
      W->WriteArray(start_address, end_address,
                    template_weights[template_index]);

      AU::Array<double> *S =
          new AU::Array<double>("EP_HDF5:template-intermediate-" +
                                std::to_string(template_index) + ".h5:/start");
      ...
      // std::vector<double> template_tstart_1D =
      // Convert2DVTo1DV(template_tstart);
      S->WriteArray(start_address, end_address,
                    template_tstart[template_index]);
  ...
  // std::cout << " template_data.size =" << template_data.size() << " ,
  // template_data[0].size = " << template_data[0].size() << " ,
  // template_data[0][0].size = " << template_data[0][0].size();

  // PrintVV("template_data[0] = ", template_data[0]);
}

template <class TT>
inline Stencil<std::vector<double>>
udf_template_match(const Stencil<TT> &iStencil) {
...
// #define TEST_SUM_SQ_INSIDE 1
  ...
  // Input pramters
  std::vector<int>
      max_offset_upper; // Size of input data for the whole chunk, zero based
  iStencil.GetOffsetUpper(max_offset_upper);
  if (ft_rank == 0 || ft_rank == (ft_size - 1))
    PrintVector("max_offset_upper = ", max_offset_upper);
  int chs_per_file_udf, lts_per_file_udf; // lts_per_file_udf is the points per
  ...
  std::vector<std::vector<double>>
      ts2d; // Buffer to stored the converted from ts_short to 2D
            // ts2d[0] is a channel ....

  // double template_tstart_max;           // temporary value for the tstart
  // size_t dx1;                           // npts2 is the size of vector for
  // cross-correlation dx1 is the start index for the points
  std::vector<std::vector<double>> xc0; // [template index][correlation]

  std::vector<std::vector<double>>
      amat1; // filter data , [channel][time points]
  std::vector<double>
      ts_temp2; // temporary value for each ch during filter, try to remove it
  ...
  if (lts_per_file_udf != (nof1 * npts0)) {
    ...
    std::cout << "Skip this chunk, it has = " << lts_per_file_udf
              << " points, but we initialize for [" << nof1 * npts0
              << "]points \n";
    // goto ReturnEmpty;
    oStencil.SetEmpty();
    if (oStencil.IsEmpty()) {
  ...
  if (!ft_rank)
    std::cout << "ReadNeighbors (s) = " << AU_WTIME - init_xcorr_t_start
              << std::endl;
  init_xcorr_t_start = AU_WTIME;

  ts2d = DasLib::Vector1D2DByColStride(chs_per_file_udf, ts_short, 2, 3);
  if (!ft_rank || ft_rank == (ft_size - 1))
    std::cout << "chs = " << ts2d.size() << ", each with " << ts2d[0].size()
              << " points\n";

  // PrintVV("ts2d = ", ts2d);
  if (!ft_rank)
    std::cout << "Vector1D2DByColStride (s) = " << AU_WTIME - init_xcorr_t_start
              << std::endl;
  init_xcorr_t_start = AU_WTIME;
  ...
  if (is_smoothhw) {
    ...
    npts1_new = round(((nof1 - 1) * npts0) / decifac) +
                round(taperwidth / dt1) + MaxVector(template_winlen) +
                MaxVectorVector(template_tstart) +
                2 * MaxVector(template_smoothhw) + 1;
  } else {
    npts1_new = round(((nof1 - 1) * npts0) / decifac) +
                round(taperwidth / dt1) + MaxVector(template_winlen) +
                MaxVectorVector(template_tstart) + 1;
  }
  if (npts1_new < npts1)
    npts1 = npts1_new;
...
#pragma omp parallel for firstprivate(ts_temp2)
  ...
  for (int ii = 0; ii < chs_per_file_udf; ii++) {
    ...
    //     chs_per_file_udf << "\n";
    ts_temp2 =
        ddff(ts2d[ii], ctap0, 10, BUTTER_A, BUTTER_B, cheby1_b, cheby1_a);
    // ts_temp2.pop_back();
  ...
  if (is_save_pre_processed_files) {
    ...
    if (is_column_major) {
      ...
      transpose(ts_temp.data(), ts_temp_column.data(), amat1.size(),
                amat1[0].size());
  ...
  // std::cout << " amat1.size =" << amat1.size() << " , amat1[0].size = " <<
  // amat1[0].size() << " \n"; PrintVV("amat1  ", amat1);
  ...
  for (int rc2 = 0; rc2 < ntemplates; rc2++) {
    temp_value = MaxVector(
        template_tstart
            [rc2]); //*(std::max_element(std::begin(template_tstart[rc2]),
                    // std::end(template_tstart[rc2])));
  ...
  if (!ft_rank)
    std::cout << "npts1 = " << npts1 << ", npts2_max = " << npts2_max
              << ", chs_per_file_udf =" << chs_per_file_udf
              << ", nchan1 = " << nchan1 << ", mpi_rank =" << ft_rank
              << ", ntemplates = " << ntemplates << ", nchan1 = " << nchan1
              << ", npts2_vector[0] = " << npts2_vector[0]
              << ", npts1_new = " << npts1_new << "\n";

  // std::deque<double> ch_window_buffer(template_winlen[0], 0);
  // firstprivate(ch_window_buffer)
  //  #if defined(_OPENMP)
  //  #endif
  ////#pragma omp parallel for
  for (int rc2 = 0; rc2 < ntemplates; rc2++) {
...
#pragma omp parallel for firstprivate(sdcn_v, dx1)
        ...
        for (int rc3 = 0; rc3 < npts2_vector[rc2]; rc3++) {
          ...
          double sum_sq = 0;
#endif
          // Replace below line with the following to find difference of two
          // version
          if (correlation_method != CORR_DOT_PRODUCT_NO_DETREND) {
            detrend_range_one_pass_std(amat1[rc1], dx1, template_winlen[rc2],
                                       ctap_template2, xmean, xsum, Sxx,
                                       sdcn_v);
          } else {
          ...
          // sdcn(amat1[rc1], sdcn_v, dx1, template_winlen[rc2],
          // ctap_template2); double temp_xcorr;
          switch (correlation_method) {
          case CORR_DOT_PRODUCT_NO_DETREND:
          case CORR_DOT_PRODUCT:
            xc0[rc2][rc3] = xc0[rc2][rc3] +
                            template_weights[rc2][rc1] *
                                dot_product(sdcn_v, template_data[rc2][rc1]);
            break;
            // xc0[rc2][rc3] = xc0[rc2][rc3] + template_weights[rc2][rc1] *
            // dot_product(sdcn_v, template_data[rc2][rc1]); break;
          case CORR_DOT_PRODUCT_NEIGHBORS:
            // xc0[rc2][rc3] = dot_product(sdcn_v, template_data[rc2][rc1]);
            correlation_per_chal[rc3] =
                dot_product(sdcn_v, template_data[rc2][rc1]);
            break;
          case CORR_XCORR_MAX:
            xc0[rc2][rc3] =
                xc0[rc2][rc3] + template_weights[rc2][rc1] *
                                    xcross_max(sdcn_v, template_data[rc2][rc1]);
            break;
          case CORR_FFT_MAX:
            if (!rc3 && !rc1) {
              ...
              std::cout << "xcross_fft = "
                        << xcross_fft(sdcn_v, template_data[rc2][rc1]) << "\n";
            }
            ...
            break;
          default:
            std::cout << "Unsupported Correlation Method code "
                      << correlation_method << "\n";
            std::cout << "Please set correlation_method = 0 (dot-product), 1 "
                         "(xcorr-max), 2 (fft-max) "
                      << correlation_method << "\n";
        ...
        if (correlation_method == CORR_DOT_PRODUCT_NEIGHBORS) {
          ...
          for (int rc3 = 0; rc3 < npts2_vector[rc2]; rc3++) {
            max_neighbors[rc3] =
                template_weights[rc2][rc1] *
                find_max_neighbors(correlation_per_chal, rc3, n_neighbors);
          }
          for (int rc3 = 0; rc3 < npts2_vector[rc2]; rc3++) {
  ...
  if (!ft_rank)
    std::cout << "sdcn (for loop of all templates ) (s) = "
              << AU_WTIME - init_xcorr_t_start << std::endl;
  init_xcorr_t_start = AU_WTIME;
...
int main(int argc, char *argv[]) {
  ...
  // std::cout << "EP_DIR:" + das_file_type + ":" + das_dir << ":" <<
  // das_h5_dataset << "\n";
  std::string A_endpoint_id;

  if (!is_input_single_file) {
    A_endpoint_id =
        "EP_DIR:" + das_file_type + ":" + das_dir + ":" + das_h5_dataset;
  } else {
  ...
  if (!is_column_major_from_config) {
    /*
     * Here we try to read
     * MeasureLengthName/SpatialResolutionName/SamplingFrequencyName or
     * nTrace/nPoint to detect the layout of the file.
     *
     */
    ...
    if (MeasureLength.empty() || SpatialResolution.empty()) {
      ...
      if (MeasureLength.empty()) {
        std::cout << "Error: Can not find out [" << MeasureLengthName << "/"
                  << SpatialResolutionName
                  << "] or [nTrace] in the file to detect organization. You "
                     "can also set [is_column_vector = true/false] in config "
                     "file to disable this detection ! \n";
        exit(-1);
      }
    ...
    if (SamplingFrequency.empty()) {
      ...
      if (SamplingFrequency.empty()) {
        std::cout << "Error: Can not find out [" << SamplingFrequencyName
                  << "] or [nPoint] in the file to detect organization. You "
                     "can also set [is_column_vector = true/false] in config "
                     "file to disable this detection ! \n";
        exit(-1);
      }
    ...
    // std::cout << "MeasureLength= " << MeasureLength << " , SpatialResolution
    // =" << SpatialResolution << ", SamplingFrequency = " << SamplingFrequency
    // << std::endl; if (!MeasureLength.empty() && !SpatialResolution.empty() &&
    // !SamplingFrequency.empty())
    //{
    //     meta_chs = std::stoi(MeasureLength) / std::stoi(SpatialResolution);
    //     meta_time_series_points = 60 * std::stoi(SamplingFrequency);
    // }
    // else
    //{
    //     std::cout << "Metadata can not be found for, " << MeasureLengthName
    //     << ", " << SpatialResolutionName << ", and " << SamplingFrequencyName
    //     << ", please specify the is_column_vector in config file, check
    //     attribute_name_*, ... " << std::endl; exit(-1);
    // }

    // std::cout << "meta_time_series_points = " << meta_time_series_points << "
    // , meta_chs =  " << meta_chs << " \n";
    if (chunk_size[0] == meta_time_series_points && chunk_size[1] == meta_chs) {
      ...
      if (!ft_rank) {
        ...
        std::cout << termcolor::magenta
                  << "Found data organization = " << termcolor::green
                  << " column vector (time by channel) \n";
        std::cout << termcolor::reset << "\n";
      }
    } else if (chunk_size[0] == meta_chs &&
               chunk_size[1] == meta_time_series_points) {
      is_column_major = false;
      if (!ft_rank) {
        std::cout << termcolor::reset << "\n";
        std::cout << termcolor::magenta
                  << "Found data organization = " << termcolor::green
                  << " row vector (channel by time)\n";
        std::cout << termcolor::reset << "\n";
      }
    } else {
      std::cout << "Metadata and data are inconsistent in the size ! "
                << std::endl;
      exit(-1);
    }
  }
  ...
  if (is_template_file_range) {
    for (int i = template_file_range_start_index;
         i <= template_file_range_end_index; i++) {
  ...
  init_xcorr();
  if (!ft_rank)
    std::cout << "init_xcorr time (s) = " << AU_WTIME - init_xcorr_t_start
              << std::endl;
  // Result data
  AU::Array<double> *B;
  if (is_output_single_file) {
    // Store into a single file
    B = new AU::Array<double>(output_type + ":" + output_file_dir + ":" +
                              output_dataset);
  } else {
    // Store into multiple file
    B = new AU::Array<double>("EP_DIR:" + output_type + ":" + output_file_dir +
                              ":" + output_dataset);
...
int read_config_file(std::string file_name, int mpi_rank) {
  ...
  template_dir =
      reader.Get("parameter", "template_dir",
                 "/Users/dbin/work/dassa/template-match/template_dir/");

  das_dir =
      reader.Get("parameter", "das_dir",
                 "/Users/dbin/work/dassa/template-match/template-match-data");
  ...
  if (temp_str_str == "short" || temp_str_str == "0") {
    ...
    AU_EXIT("Don't understand the [input_data_type] in config file. It can "
            "take short/double/float or 0/1/2\n");
  }

  std::string temp_str =
      reader.Get("parameter", "is_input_single_file", "false");
  ...
  if (is_das_input_search_rgx) {
    das_input_search_rgx =
        reader.Get("parameter", "das_input_search_rgx", "^(.*)[1234]\\.tdms$");
  }
  ...
  if (is_template_input_search_rgx) {
    template_input_search_rgx = reader.Get(
        "parameter", "template_input_search_rgx", "(ci37327652|ci37329164)");
  }

  std::string is_file_range_str =
      reader.Get("parameter", "is_das_file_range", "false");
  ...
  if (is_file_range) {
    file_range_start_index =
        reader.GetInteger("parameter", "das_file_range_start_index", 0);
    file_range_end_index =
        reader.GetInteger("parameter", "das_file_range_end_index", 1);
  }
  ...
  if (is_channel_range) {
    channel_range_start =
        reader.GetInteger("parameter", "channel_range_start", 0);
  ...
  if (is_channel_stride) {
    channel_stride_size =
        reader.GetInteger("parameter", "channel_stride_size", 1);
  }
  ...
  if (correlation_method < 0 || correlation_method > 4) {
    AU_EXIT("Don't understand the correlation_method's value " +
            correlation_method);
  }
  ...
  std::string is_template_file_range_str =
      reader.Get("parameter", "is_template_file_range", "false");
  if (is_template_file_range_str == "false" ||
      is_template_file_range_str == "0") {
    is_template_file_range = false;
  } else if (is_template_file_range_str == "true" ||
             is_template_file_range_str == "1") {
    is_template_file_range = true;
  } else {
    AU_EXIT("Don't read the is_template_file_range's value " +
            is_template_file_range);
  }

  if (is_template_file_range) {
    template_file_range_start_index =
        reader.GetInteger("parameter", "template_file_range_start_index", 0);
    template_file_range_end_index =
        reader.GetInteger("parameter", "template_file_range_end_index", 1);
  }

  MeasureLengthName = reader.Get("parameter", "attribute_name_measure_length",
                                 "MeasureLength[m]");
  SpatialResolutionName = reader.Get(
      "parameter", "attribute_name_spatial_resolution", "SpatialResolution[m]");
  SamplingFrequencyName =
      reader.Get("parameter", "attribute_name_sampling_frequency",
                 "SamplingFrequency[Hz]");

  taperwidth = reader.GetInteger("parameter", "taperwidth", 5);

  n_files_to_concatenate =
      reader.GetInteger("parameter", "n_files_to_concatenate", 1);
  ...
  std::string is_template_list_output_file_str =
      reader.Get("parameter", "is_template_list_output_file", "false");
  if (is_template_list_output_file_str == "false" ||
      is_template_list_output_file_str == "0") {
    is_template_list_output_file = false;
  } else if (is_template_list_output_file_str == "true" ||
             is_template_list_output_file_str == "1") {
    is_template_list_output_file = true;
  } else {
    AU_EXIT("Don't read the is_template_list_output_file's value " +
            is_template_list_output_file);
  }

  if (is_template_list_output_file)
    template_list_output_file = reader.Get(
        "parameter", "template_list_output_file", "template_final_list.txt");

  std::string is_input_template_file_list_str =
      reader.Get("parameter", "is_input_template_file_list", "false");
  if (is_input_template_file_list_str == "false" ||
      is_input_template_file_list_str == "0") {
    is_input_template_file_list = false;
  } else if (is_input_template_file_list_str == "true" ||
             is_input_template_file_list_str == "1") {
    is_input_template_file_list = true;
  } else {
    AU_EXIT("Don't read the is_input_template_file_list's value " +
            is_input_template_file_list);
  }

  if (is_input_template_file_list) {
    input_template_file_list =
        reader.Get("parameter", "input_template_file_list",
                   "input_template_file_list.txt");
    input_template_files = read_template_file_list(input_template_file_list);
  }

  output_file_dir =
      reader.Get("parameter", "output_file_dir", "./tdms-dir-dec/test.h5");

  output_dataset = reader.Get("parameter", "output_dataset", "/data");

  temp_str =
      reader.Get("parameter", "is_dir_output_match_replace_rgx", "false");

  is_dir_output_match_replace_rgx = (temp_str == "false") ? false : true;

  if (is_dir_output_match_replace_rgx) {
    dir_output_match_rgx =
        reader.Get("parameter", "output_file_regex_match", "^(.*)\\.tdms$");

    dir_output_replace_rgx =
        reader.Get("parameter", "output_file_regex_replace", "$1.h5");
  }
  ...
  if (omp_num_threads_p < 0) {
    AU_EXIT("omp_num_threads must be positive integer : " +
            std::to_string(omp_num_threads_p));
  }
  ...
  if (!mpi_rank) {
    ...
    std::cout << termcolor::magenta
              << "\n        template_dir = " << termcolor::green
              << template_dir;

    std::cout << termcolor::magenta
              << "\n        das_dir = " << termcolor::green << das_dir;
    std::cout << termcolor::magenta
              << "\n        das_file_type = " << termcolor::green
              << das_file_type;
    if (input_data_type == 0) {
      std::cout << termcolor::magenta
                << "\n        input_data_type = " << termcolor::green
                << "short";
    } else if (input_data_type == 1) {
      std::cout << termcolor::magenta
                << "\n        input_data_type = " << termcolor::green
    ...
    if (is_column_major_from_config) {
      if (is_column_major == true) {
        std::cout << termcolor::magenta
                  << "\n        is_column_vector = " << termcolor::green
                  << "true";
      } else {
        std::cout << termcolor::magenta
                  << "\n        is_column_vector = " << termcolor::green
    ...
    std::cout << termcolor::magenta
              << "\n        is_template_file_range = " << termcolor::green
              << is_template_file_range;
    if (is_template_file_range) {
      std::cout << termcolor::magenta
                << "\n        template_file_range_start_index = "
                << termcolor::green << template_file_range_start_index;
      std::cout << termcolor::magenta
                << "\n        template_file_range_end_index = "
                << termcolor::green << template_file_range_end_index;
    }

    std::cout << termcolor::magenta
              << "\n        is_das_file_range = " << termcolor::green
              << is_file_range;
    if (is_file_range) {
      std::cout << termcolor::magenta
                << "\n        das_file_range_start_index = " << termcolor::green
                << file_range_start_index;
      std::cout << termcolor::magenta
                << "\n        das_file_range_end_index = " << termcolor::green
                << file_range_end_index;
    }

    if (is_channel_range) {
      std::cout << termcolor::magenta
                << "\n        is_channel_range = " << termcolor::green
                << "true";
      std::cout << termcolor::magenta
                << "\n        channel_range_start = " << termcolor::green
                << channel_range_start;
      std::cout << termcolor::magenta
                << "\n        channel_range_end = " << termcolor::green
                << channel_range_end;
    } else {
      std::cout << termcolor::magenta
                << "\n        is_channel_range = " << termcolor::green
    ...
    if (is_channel_stride) {
      std::cout << termcolor::magenta
                << "\n        is_channel_stride = " << termcolor::green
                << "true";
      std::cout << termcolor::magenta
                << "\n        channel_stride_size = " << termcolor::green
                << channel_stride_size;
    } else {
      std::cout << termcolor::magenta
                << "\n        is_channel_stride = " << termcolor::green
    ...
    if (is_das_input_search_rgx) {
      std::cout << termcolor::magenta
                << "\n        das_input_search_rgx = " << termcolor::green
                << das_input_search_rgx;
    }
    std::cout << termcolor::magenta
              << "\n        n_files_to_concatenate = " << termcolor::green
              << n_files_to_concatenate;

    std::cout << termcolor::blue << "\n\n Runtime parameters: ";
    // std::cout << termcolor::magenta << "\n\n        lts_per_file = " <<
    // termcolor::green << lts_per_file; std::cout << termcolor::magenta << "\n
    ...
    std::cout << termcolor::magenta
              << "\n        OpenMP_num_threads = " << termcolor::green
              << omp_num_threads_p;
    switch (correlation_method) {
    case CORR_DOT_PRODUCT:
      std::cout << termcolor::magenta
                << "\n        correlation_method = " << termcolor::green
                << CORR_DOT_PRODUCT << " dot_product";
      break;
    case CORR_DOT_PRODUCT_NEIGHBORS:
      std::cout << termcolor::magenta
                << "\n        correlation_method = " << termcolor::green
                << CORR_DOT_PRODUCT_NEIGHBORS << " dot_product_neighbors";
      break;
    case CORR_DOT_PRODUCT_NO_DETREND:
      std::cout << termcolor::magenta
                << "\n        correlation_method = " << termcolor::green
                << CORR_DOT_PRODUCT_NO_DETREND << " dot_product_no_detrend";
      break;
    case CORR_XCORR_MAX:
      std::cout << termcolor::magenta
                << "\n        correlation_method = " << termcolor::green
                << CORR_XCORR_MAX << " xcorr_max";
      break;
    case CORR_FFT_MAX:
      std::cout << termcolor::magenta
                << "\n        correlation_method = " << termcolor::green
                << CORR_FFT_MAX << " fft_max";
      break;
    default:
      break;
    }
    ...
    if (is_input_template_file_list) {
      std::cout << termcolor::magenta
                << "\n     input_template_file_list = " << termcolor::green
                << input_template_file_list;
      ...
      if (input_template_files_size > 2) {
        ...
        std::cout << termcolor::magenta << "\n                                "
                  << termcolor::green
                  << input_template_files[input_template_files_size - 1];
    ...
    if (is_template_list_output_file)
      std::cout << termcolor::magenta
                << "\n    template_list_output_file = " << termcolor::green
                << template_list_output_file;
    std::cout << termcolor::magenta
              << "\n        is_output_single_file = " << termcolor::green
              << is_output_single_file;
    std::cout << termcolor::magenta
              << "\n        output_type = " << termcolor::green << output_type;
    std::cout << termcolor::magenta
              << "\n        output_file_dir = " << termcolor::green
              << output_file_dir;
    std::cout << termcolor::magenta
              << "\n        output_dataset = " << termcolor::green
              << output_dataset;
    std::cout << termcolor::magenta
              << "\n        taperwidth     = " << termcolor::green
              << taperwidth;

    if (is_dir_output_match_replace_rgx) {
      std::cout << termcolor::magenta
                << "\n        dir_output_match_rgx = " << termcolor::green
                << dir_output_match_rgx;
      std::cout << termcolor::magenta
                << "\n        dir_output_replace_rgx = " << termcolor::green
                << dir_output_replace_rgx;
    }
...
void all_gather_vector(const std::vector<double> &v_to_send,
  ...
  MPI_Allgather(&local_sum, 1, MPI_INT, local_sum_vector.data(), 1, MPI_INT,
                MPI_COMM_WORLD);
  ...
  MPI_Allgatherv(v_to_send.data(), local_sum, MPI_DOUBLE, v_to_receive.data(),
                 local_sum_vector.data(), displacements.data(), MPI_DOUBLE,
                 MPI_COMM_WORLD);
}
