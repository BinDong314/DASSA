...
template <typename T> void FreeVectorSpace(T &t) {
  T tmp;
  t.swap(tmp);
}
...
void init_xcorr() {
  ...
  for (int rc2 = 0; rc2 < ntemplates_to_go; rc2++) {
    ...
    T_ts2d = DasLib::Vector1D2DByColStride(
        T_chs, T_h5_data, channel_stride_start,
        channel_stride_size); // filter the data starting at ch (2-1) and every
                              // 3 chs std::cout << "T_ts2d.size() = " <<
                              // T_ts2d.size() << "\n";
...
#pragma omp parallel for
    ...
    for (int i = 0; i < T_ts2d.size(); i++) {
      ...
      // PrintVector("Before ddff T_ts2d[i] = ", T_ts2d[i]);
...
// #define TEST_SUM_SQ_INSIDE 1
  ...
  ts2d = DasLib::Vector1D2DByColStride(
      chs_per_file_udf, ts_short, channel_stride_start, channel_stride_size);
  if (!ft_rank || ft_rank == (ft_size - 1))
    std::cout << "chs = " << ts2d.size() << ", each with " << ts2d[0].size()
              << " points\n";

  FreeVectorSpace(ts_short);
...
int read_config_file(std::string file_name, int mpi_rank) {
  ...
  if (is_channel_stride) {
    channel_stride_start =
        reader.GetInteger("parameter", "channel_stride_start", 2);
    channel_stride_size =
        reader.GetInteger("parameter", "channel_stride_size", 3);
  }
  ...
  if (!mpi_rank) {
    ...
    if (is_channel_stride) {
      std::cout << termcolor::magenta
                ...
                << "true";
      std::cout << termcolor::magenta
                << "\n        channel_stride_start = " << termcolor::green
                << channel_stride_start;
      std::cout << termcolor::magenta
