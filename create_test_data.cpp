...
void print_help()
{
    const char *msg = "Usage: %s [OPTION] \n\
          ...
          -c number of cores. \n\
          -m attach some random tag(medata)  \n\
...
int main(int argc, char **argv)
{
    ...
    int has_metadata_flag = 0;
#ifdef SDS_CLIENT_MPI
    MPI_Comm comm = MPI_COMM_WORLD;
    MPI_Info info = MPI_INFO_NULL;
#endif

    while ((c = getopt(argc, argv, "f:g:d:n:c:s:ht:rpa:i:m")) != -1)
        switch (c)
        {
        case 'f':
    ...
    //metadata
    if (has_metadata_flag)
    {
        std::string fname_cpp(filename);
        std::string dname_cpp(dsetname);
        std::string gname_cpp(groupname);
        std::cout << "fname_cpp = " << fname_cpp << ", dname_cpp =  " << dname_cpp << "\n";
        AU::Array<float> *A = new Array<float>("EP_HDF5:" + fname_cpp + ":" + dname_cpp);
        std::string tag_value1 = "tag1_value";
        A->SetTag("Tag1", tag_value1);

        std::string tag_value2 = "100";
        A->SetTag("Tag2", tag_value2);

        std::string tag_value3 = "100.100";
        A->SetTag("Tag3", tag_value3);
