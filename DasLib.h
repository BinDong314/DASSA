...
namespace DasLib {
template <typename T>
inline T find_max_neighbors(const std::vector<T> &vec, int i,
                            int numNeighbors) {
  int n = vec.size();
...
template <class T>
inline void xcross(const std::vector<T> &X, const std::vector<T> &Y,
                   std::vector<double> &xcross_result) {
...
template <class T> inline T MaxVector(std::vector<T> &v) {
  return *std::max_element(v.begin(), v.end());
}

template <class T> inline T MaxVectorVector(std::vector<std::vector<T>> &v) {
  std::vector<T> max_v;
  max_v.resize(v.size());
...
// To be complete
template <class T>
int decimate(std::vector<T> &v, const int r,
             const std::vector<double> &cheby1_b,
             const std::vector<double> &cheby1_a) {
  size_t nd = v.size();
  ...
  // std::cout << "nd =" << nd << ", nbeg = " << nbeg << " , nout = " << nout <<
  // " , r = " << r << "\n";
  v.resize(nout);
  size_t v_i = 0;
 ...
 */
template <class T>
inline std::vector<T> spacedecimate(const std::vector<std::vector<T>> &data2d,
                                    const int start_row, const int end_row,
                                    const std::string stat_operator_str) {
  std::vector<T> result;
  ...
  for (std::size_t j = 0; j < cols; j++) {
    if (stat_operator_str == "mean" || stat_operator_str == "average" ||
        stat_operator_str == "ave") {
      stat_temp = 0;
...
// template <typename T>
inline double sum_weight(const std::vector<double> &v,
  ...
  for (size_t i = 0; i < n; i++) {
    // std::cout << i << ",  sum = " << sum << ", v[i] = " << v[i] << ", w[i]= "
    // << w[i] << "\n";
    sum = sum + v[i] * w[i];
  }
  // std::cout << "sum =" << sum << "\n";
...
// template <typename T>
inline void sum_weight_by_time(const std::vector<std::vector<double>> &v,
                               const std::vector<double> &w,
...
template <typename T>
inline void PrintVectorDebug(std::string name, std::vector<T> v) {
  int n = v.size();
  if (name != "")
    std::cout << "PrintVectorDebug, size=" << v.size() << ", " << name << ": ";
...
template <typename T>
inline std::vector<T>
ddff(std::vector<T> &in_vec, const std::vector<double> &ctap,
     const int decimate_factor, const std::vector<double> &filtfilt_B,
     const std::vector<double> &filtfilt_A,
     const std::vector<double> &decimate_cheby1_b,
     const std::vector<double> &decimate_cheby1_a) {
...
template <typename T> inline void norm_matlab(vector<T> &v) {
  T x = 0;
  for (size_t i = 0; i < v.size(); i++)
    x = x + v[i] * v[i];
  T sqrt_x = sqrt(x);
...
/*   subset, detrend, ctap, norm
    slice(atemp2, template_tstart[rc2][i] - 1, template_tstart[rc2][i] - 1 +
   template_winlen[rc2] - 1); std::cout << " atemp2.size() = " << atemp2.size()
   << ", template_tstart[rc2][i] = " << template_tstart[rc2][i] << ",
   template_winlen[rc2] = " << template_winlen[rc2] << ",  ctap_template2.size=
 ...
 */
template <typename T>
inline void sdcn_old(const std::vector<T> &v, std::vector<T> &v_out,
                     const size_t subset_start, const size_t subset_count,
                     const std::vector<T> &ctap) {
  // slice(atemp2, template_tstart[rc2][i] - 1, template_tstart[rc2][i] - 1 +
  // template_winlen[rc2] - 1); std::vector<T>::const_iterator first = v.begin()
  // + subset_start; std::vector<T>::const_iterator last = v.begin() +
  // subset_start + subset_count; std::cout << "subset_start = " << subset_start
  // << ", subset_count = " << subset_count << "\n";
  std::vector<T> newV(v.begin() + subset_start,
                      v.begin() + subset_start + subset_count);
  detrend(newV.data(), newV.size());
  // std::cout << "newV.size() = " << newV.size() << ", ctap = " << ctap.size()
  // << "\n"; VectorElementMulti(newV, ctap); double atemp2_norm =
  // norm_matlab(newV); VectorDivideByScalar(newV, atemp2_norm);
  // norm_matlab(newV);
...
template <typename T>
inline void sdcn(const std::vector<T> &v, std::vector<T> &v_out,
                 const size_t subset_start, const size_t subset_count,
                 const std::vector<T> &ctap) {
  // slice(atemp2, template_tstart[rc2][i] - 1, template_tstart[rc2][i] - 1 +
  // template_winlen[rc2] - 1); std::vector<T>::const_iterator first = v.begin()
  ...
  // std::vector<T> newV(v.begin() + subset_start, v.begin() + subset_start +
  // subset_count); detrend(newV.data(), newV.size()); detrend(v_out.data(),
  // v_out.size()); PrintVector("Before detrend_range v_out =", v);
...
template <typename T>
inline T dot_product(const std::vector<T> &vector_a,
...
int ButterLow(int n, double fcf, std::vector<double> &A,
              std::vector<double> &B) {
...
int ButterPass(int n, const std::vector<double> &fcf, std::vector<double> &A,
               std::vector<double> &B) {
  int sff = 1;  // scale flag: 1 to scale, 0 to not scale ccof
  int i;        // loop variables
...
template <class T> inline T Rms(const std::vector<T> &x) {
  T sum = 0;
  int n = x.size();
  for (int i = 0; i < n; i++)
    sum += pow(x[i], 2);
...
template <class T> inline T Mean(std::vector<T> &v) {
  T sum = 0;
...
template <class T> inline void DeleteMedian(std::vector<std::vector<T>> &v) {
  int rows = v.size();
  int cols = v[0].size();
 ...
 */
template <class T>
inline std::vector<T> TimeSubset(std::vector<T> &v, double start_t,
                                 double end_t, double sub_start_t,
                                 double sub_end_t, double smaple_rate) {
  size_t sub_start_index = round((sub_start_t - start_t) / smaple_rate);
  size_t sub_end_index = round((sub_end_t - start_t) / smaple_rate);

  // std::cout << "sub_start_index = " << sub_start_index << ", sub_end_index =
  // " << sub_end_index << "\n";
 ...
 * @return size_t
 */
inline size_t InferTimeSubsetSize(double start_t, double end_t,
                                  double sub_start_t, double sub_end_t,
                                  double smaple_rate) {
...
inline std::vector<std::vector<T2>> Vector1D2D(size_t cols,
                                               const std::vector<T1> &data1d) {
 ...
 */
template <class T1, class T2 = double>
inline std::vector<std::vector<T2>>
Vector1D2DByCol(size_t cols, const std::vector<T1> &data1d) {
  std::vector<std::vector<T2>> result;
  size_t rows = data1d.size() / cols;
  result.resize(cols);
...
template <class T1, class T2 = double>
inline std::vector<std::vector<T2>>
Vector1D2DByColStride(size_t cols, const std::vector<T1> &data1d,
                      const int start, const int stride) {
  std::vector<std::vector<T2>> result;

  size_t rows = data1d.size() / cols;            // the number of rows
  size_t new_cols = (cols - start) / stride + 1; // (6912 - 2)/3 + 1
  result.resize(new_cols);

  // std::cout << "cols = " << cols << ", start = " << start
  //           << ", stride =" << stride << ", New size = (" << rows << " , "
  //           << new_cols << " )\n";
...
template <class T1, class T2 = double>
inline std::vector<std::vector<T2>>
ConvertVector1DTo2D(const std::vector<T1> &data1d, const size_t rows,
                    const size_t cols, const bool row_layout_flag) {
  std::vector<std::vector<T2>> result;
  if (row_layout_flag) {
    // size_t rows = data1d.size() / cols;
    result.resize(rows);
...
/*
template <class T>
inline std::vector<std::vector<T>> Vector1D2D(size_t cols, std::vector<T>
&data1d)
{
...
template <class T>
inline std::vector<T>
Convert3DVTo1DV(std::vector<std::vector<std::vector<T>>> &data3d) {
  std::vector<T> result;
...
Convert1DVTo3DV(std::vector<T> data1d, size_t dim_0_size, size_t dim_1_size,
                size_t dim_2_size) {
 ...
 * @return std::vector<T>
 */
template <typename T> void slice(std::vector<T> &v, int m, int n) {
...
template <typename T> std::vector<T> slice2(std::vector<T> &v, int m, int n) {
  auto first = v.cbegin() + m;
  auto last = v.cbegin() + n + 1;
  std::vector<T> vec(first, last);
  return vec;
...
template <class T>
inline bool CausalityFlagging(std::vector<std::vector<T>> &v, double tmin,
                              double tmax, double fmax, double start_t,
                              double end_t, double smaple_rate,
                              double CausalityFlagging_ButterLow_order,
                              double CausalityFlagging_ButterLow_fcf) {
  int N = v.size();
  ...
  ButterLow(CausalityFlagging_ButterLow_order, CausalityFlagging_ButterLow_fcf,
            A, B);
...
PrintScalar("tmax", tmax);*/
  ...
  for (int i = 0; i < N; i++) {
    ...
    tv3[i] = TimeSubset(tv2[i], -tmax, tmax, tmin, tmax * 0.9,
                        smaple_rate); // causal
    tv4[i] = TimeSubset(tv2[i], -tmax, tmax, -tmax * 0.9, -tmin,
                        smaple_rate); // acausal
  }
  ...
  // std::cout << "rms_causal_mean = " << rms_causal_mean << ", rms_causal_mean
  // =" << rms_causal_mean << "\n";
...
inline std::vector<std::complex<T>> Hilbert(std::vector<T> &in) {
  ...
  fftw_complex *in_temp =
      (fftw_complex *)fftw_alloc_complex(sizeof(fftw_complex) * INN);
  ...
  // micro_time = AU_WTIME;
  fftw_plan pf =
      fftw_plan_dft_1d(INN, in_temp, out_temp, FFTW_FORWARD, FFTW_ESTIMATE);
  ...
  fftw_plan pf2 =
      fftw_plan_dft_1d(INN, out_temp, in_temp, FFTW_BACKWARD, FFTW_ESTIMATE);
  fftw_execute(pf2);
...
template <class T>
inline std::vector<std::vector<std::complex<T>>>
HilbertVector(std::vector<std::vector<T>> &in) {
  ...
  // std::cout << "INN =" << INN << "\n";
  fftw_complex *in_temp =
      (fftw_complex *)fftw_alloc_complex(sizeof(fftw_complex) * INN_TOTOAL);
  // std::memset(in_temp, 0, sizeof(fftw_complex) * INN);
  ...
  // micro_time = AU_WTIME;
  //     fftw_plan pf = fftw_plan_dft_1d(INN, in_temp, out_temp, FFTW_FORWARD,
  //     FFTW_ESTIMATE);
  rank = 1;
  howmany = INN_ROW;
  istride = 1;
  ...
  fftw_plan pf = fftw_plan_many_dft(rank, &col, howmany, in_temp, NULL, istride,
                                    idist, out_temp, NULL, ostride, odist,
  ...
  for (size_t row = 0; row < INN_ROW; row++) {
    ...
    memset(&out_temp[start_offset + HN + 1][0], 0,
           numRem * sizeof(fftw_complex));
  }

  // std::memset(in_temp, 0, sizeof(fftw_complex) * INN);
  // fftw_plan pf2 = fftw_plan_dft_1d(INN, out_temp, in_temp, FFTW_BACKWARD,
  // FFTW_ESTIMATE);

  fftw_plan pf2 = fftw_plan_many_dft(rank, &col, howmany, out_temp, NULL,
                                     istride, idist, in_temp, NULL, ostride,
...
template <class T>
inline std::vector<std::vector<std::complex<T>>>
HilbertVectorIP(const std::vector<std::vector<T>> &in) {
  size_t INN_ROW = in.size();
  size_t INN_COL = in[0].size();
  size_t INN_TOTOAL = INN_ROW * INN_COL;
  int rank, col, howmany, istride, idist, ostride, odist;
  ...
  // std::cout << "INN =" << INN << "\n";
  // fftw_complex *in_temp = (fftw_complex
  // *)fftw_alloc_complex(sizeof(fftw_complex) * INN_TOTOAL);
  // std::memset(in_temp, 0, sizeof(fftw_complex) * INN);
  fftw_complex *out_temp =
      (fftw_complex *)fftw_alloc_complex(sizeof(fftw_complex) * INN_TOTOAL);
  if (out_temp == NULL) {
  ...
  fftw_plan pf = fftw_plan_many_dft(rank, &col, howmany, out_temp, NULL,
                                    istride, idist, out_temp, NULL, ostride,
...
template <class T>
inline std::vector<std::vector<std::complex<T>>>
instanPhaseEstimatorVector(std::vector<std::vector<T>> &v) {
